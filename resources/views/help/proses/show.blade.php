                                                    ...
                                                    @if(str_contains($komentarLampiran->tipe_file, 'image'))
                                                        <div class="mt-2">
                                                            <div class="relative inline-block group">
                                                                <img src="{{ route('help.proses.lampiran.preview', ['lampiran' => $komentarLampiran->id]) }}?thumb=true" 
                                                            ...
                                                            <div class="mt-1 flex items-center">
                                                                ...
                                                                <a href="{{ route('help.proses.lampiran.download', ['lampiran' => $komentarLampiran->id]) }}" 
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            ...
                            <div class="group relative cursor-pointer" onclick="previewFile('{{ $lampiran->id }}', '{{ $lampiran->nama_file }}')">
                                ...
                                <div class="aspect-square overflow-hidden rounded-lg border border-gray-300 bg-gray-100">
                                    <img src="{{ route('help.proses.lampiran.preview', ['lampiran' => $lampiran->id]) }}?thumb=true" 
                                ...
                                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 text-white opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button onclick="event.stopPropagation(); window.open('{{ route('help.proses.lampiran.download', ['lampiran' => $lampiran->id]) }}', '_blank')" 
                    ...
                    <div>
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
                                ...
                                <div class="ml-3">
                                    <a href="{{ route('help.proses.lampiran.download', ['lampiran' => $lampiran->id]) }}" 
...
function previewFile(id, fileName) {
    ...
    const previewUrl = "{{ route('help.proses.lampiran.preview', ['lampiran' => ':id']) }}".replace(':id', id) + '?_=' + new Date().getTime();
...
function downloadCurrentPreview() {
    if (currentFileId) {
        const downloadUrl = "{{ route('help.proses.lampiran.download', ['lampiran' => ':id']) }}".replace(':id', currentFileId);
